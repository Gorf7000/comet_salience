...
6. Flags, but does not prematurely delete, boundary cases such as Halley, Biela, faint/telescopic apparitions, retrospective pre-discovery appearances, and ambiguous source records.
...
- comet photometric parameters if available;
...
- `spectacle_mag6_excess` privileges short periods of exceptional brightness over long periods of marginal visibility.
...
10. List of ambiguous identifiers.
11. List of ambiguous statuses.
12. List of rows requiring manual review.
13. List of boundary cases, including Halley and Biela if present.
14. Summary of magnitude-quality labels.
15. Any API failures, missing data, or assumptions.

The audit report should explicitly state that city/topocentric visibility is not implemented in this increment.
...
4. Log failed source requests.
5. Preserve raw status values and raw identifiers.
6. Preserve manual-review flags.
7. Document all assumptions in the audit report.
8. Use deterministic output file names.
9. Include enough comments/docstrings that a future coding session can extend the pipeline.
...
- If a JPL match exists but no magnitude is available, preserve the row and set `failed_light_curve = true` or `missing_magnitude_model = true`.
